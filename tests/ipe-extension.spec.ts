...
<html>
  <head>
    <title>Abdulla Ajlan - Indeed Profile</title>
    <script>
  ...
  <body>
    <main>
      ...
      <p>AA abdull</p>
      <p>a@aajlan.net</p>
      <p>abdulla@aajlan.net</p>
      <p>+973 6669 9553</p>
      <p>Bahrain (Open to UAE), 244, BH</p>
      <p>Employers can find you</p>
      <section>
        <h2>Resume</h2>
        <article>
          <p>PDF</p>
          <p>Abdulla Ajlan - Senior HSSE and Security Executive.pdf</p>
          <p>Added today</p>
        </article>
      </section>
      ...
      <div class="profile-row" style="cursor: pointer;" onclick="openDetail('Ready to work', 'Let employers know that you are available to start working as soon as possible.', ['Available immediately', 'Open to UAE'])">
        <h3>Ready to work</h3>
...
test("exports visible user profile data from an Indeed profile page", async () => {
  ...
  try {
    ...
    await expect(page.getByRole("heading", { name: "Abdulla Ajlan" })).toBeVisible();
    ...
    expect(response.data.profile.name).toBe("Abdulla Ajlan");
    expect(response.data.profile.contact.email).toBe("abdulla@aajlan.net");
    expect(response.data.resume.title).toBe("Abdulla Ajlan - Senior HSSE and Security Executive.pdf");
    expect(response.data.sections.map((section: { heading: string }) => section.heading)).not.toContain("Abdulla Ajlan");
    ...
    expect(jobPreferences.items).toEqual(expect.arrayContaining(["Desired pay: BHD 1,800+", "Schedule: Full-time"]));
    ...
    expect(readyToWork.items).toEqual(expect.arrayContaining(["Available immediately", "Open to UAE"]));
    ...
    await expect(previewPage.locator("iframe.print-frame")).toHaveAttribute("srcdoc", /Abdulla Ajlan/);
  } finally {
    await context.close();
